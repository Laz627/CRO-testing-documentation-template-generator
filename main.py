# Create a production-ready Streamlit app file that matches the user's requirements.
# The app includes:
# - Guided 2-phase workflow (Idea Builder with optional OpenAI assist; Spec & Validate)
# - KPI presets and dropdowns
# - EASIER scoring (equal weights)
# - Power & duration estimator
# - Catalog with CSV/XLSX export
# - Word (.docx) export that mirrors the provided template headings/order
# - Manual data entry only
#
# The app is a single-file Streamlit app saved as /mnt/data/cro_self_service_tool.py
...
def init_session():
    if "catalog" not in st.session_state:
        st.session_state.catalog = []  # list of dicts
...
def mde_from_relative(baseline_cr: float, relative_lift_pct: float) -> float:
    \"\"\"Return absolute MDE as decimal given baseline CR and relative % lift.\"\"\"
...
def sample_size_per_variant(baseline_cr: float, mde_abs: float, alpha: float = 0.05, power: float = 0.8) -> Optional[int]:
    ...
    Using a standard approximation for two-sided test.
    ...
    try:
        ...
        p_bar = (p1 + p2) / 2.0
        # Pooled standard error assumption
...
def estimate_duration_weeks(weekly_sessions: int, variants: int, sample_per_variant: int, allocation: float = 1.0) -> Optional[float]:
    \"\"\"Estimate duration in weeks given traffic and sample size needs.\"\"\"
    try:
...
def to_catalog_row(form_data: Dict[str, Any]) -> Dict[str, Any]:
    \"\"\"Map form data to the Test Log schema you shared.\"\"\"
...
def export_word_doc(form_data: Dict[str, Any]) -> Optional[bytes]:
    \"\"\"Create a Word doc mirroring the template headings/order you provided.\"\"\"
    if Document is None:
        return None
    ...
    # Meta
    p = doc.add_paragraph()
    p.add_run("Test ID: ").bold = True
    p.add_run(form_data.get("test_id", ""))
    p.add_run("\nOwner: ").bold = True
    p.add_run(form_data.get("owner", ""))
    p.add_run("\nStatus: ").bold = True
    p.add_run(form_data.get("status", ""))
    p.add_run("\nDate Created: ").bold = True
    p.add_run(datetime.now().strftime("%Y-%m-%d"))
    p.add_run("\nPage URL(s): ").bold = True
    p.add_run(form_data.get("page_urls", ""))

    # Hypothesis
    doc.add_heading("Hypothesis", level=2)
    doc.add_paragraph(form_data.get("hypothesis", ""))
    ...
    # Audience / Placement
    doc.add_heading("Audience / Placement", level=2)
    doc.add_paragraph(form_data.get("audience_placement", ""))

    # Variant Scaffolding
    doc.add_heading("Variant Summary", level=2)
    doc.add_paragraph(form_data.get("variant_summary", ""))

    # EASIER
    doc.add_heading("EASIER Scoring", level=2)
    tbl = doc.add_table(rows=2, cols=6)
    hdr_cells = tbl.rows[0].cells
    for i, comp in enumerate(EASIER_COMPONENTS):
        hdr_cells[i].text = comp
    val_cells = tbl.rows[1].cells
    for i, comp in enumerate(EASIER_COMPONENTS):
        val_cells[i].text = str(form_data.get("easier", {}).get(comp, ""))
    ...
    for label, done in checks.items():
        doc.add_paragraph(f"[{'x' if done else ' '}] {label}")

    # Notes
    doc.add_heading("Notes / Risks / Dependencies", level=2)
    doc.add_paragraph(form_data.get("notes", ""))

    # Power Estimator (if calculated)
...
def idea_builder() -> Optional[Dict[str, Any]]:
    ...
    with colA:
        use_ai = st.checkbox("Use OpenAI to generate candidate ideas", value=False)
    with colB:
        temperature = st.slider("Creativity", 0.0, 1.0, 0.4) if use_ai else 0.0
        ...
        generated = [
            {
                ...
                "hypothesis": f"By refining {test_type.lower()} to better align with user intent for {page_url or 'the selected page'}, "
                              f"we expect an uplift in the primary KPI.",
            ...
            },
            {
                ...
                "hypothesis": f"By removing/reducing a friction element tied to {test_type.lower()}, "
                              f"users will progress more reliably to the form and complete it.",
                "suggested_primary_kpi": "RTA Form Completes (Lead Form Submits)",
                "suggested_secondary_kpis": ["Page-to-Form CTR", "Time to First Input"],
            },
            {
                "test_name": f"{test_type}: Trust & Proof",
                "hypothesis": f"By adding clear trust signals relevant to {test_type.lower()}, "
                              f"we reduce uncertainty and increase completions.",
                "suggested_primary_kpi": "RTA Form Completes (Lead Form Submits)",
                "suggested_secondary_kpis": ["RTA Form Starts (Lead Form Starts)", "Bounce Rate"],
            },
        ]

        # If OpenAI key is provided via st.secrets or env, try to refine ideas.
        if use_ai:
            try:
...
Page URL: {page_url}
Test Type: {test_type}
For each idea, output JSON with keys: test_name, hypothesis, suggested_primary_kpi, suggested_secondary_kpis (list).
\"\"\"
                    # Using the legacy completions interface for broad compatibility
                    # If newer clients are available, update accordingly.
                    ...
                    st.info("No OpenAI API key found. Using built-in suggestions.")
...
def create_test_form(prefill: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    with col1:
        test_id = st.text_input("Test ID (user-entered)", value=prefill.get("test_id","") if prefill else "")
    with col2:
        test_name = st.text_input("Test Name", value=prefill.get("test_name","") if prefill else "")
    ...
    with col5:
        weekly_sessions = st.number_input("Weekly Sessions (est.)", min_value=0, value=0, step=100)
    with col6:
        baseline_cr = st.number_input("Current Conversion Rate (0.0–1.0)", min_value=0.0, max_value=1.0, value=0.0, step=0.001, format="%.3f")
    with col7:
        baseline_sample_size = st.number_input("Baseline Sample Size (if known)", min_value=0, value=0, step=100)
    ...
    with col8:
        owner = st.text_input("Owner (Requester)")
    with col9:
        status = st.selectbox("Status", STATUS_OPTIONS, index=0)
    with col10:
        ready_for_dev = st.checkbox("Mark Ready for Dev", value=False)
...
def validate_block(form_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    with col1:
        variants = st.number_input("Number of Variants (incl. Control)", min_value=2, value=2, step=1)
    with col2:
        alpha = st.number_input("Alpha (Type I error)", min_value=0.001, max_value=0.2, value=0.05, step=0.001, format="%.3f")
    with col3:
        power = st.number_input("Desired Power", min_value=0.5, max_value=0.99, value=0.8, step=0.01, format="%.2f")

    col4, col5 = st.columns(2)
    with col4:
        mde_input_mode = st.radio("MDE Input", ["Relative % lift", "Absolute (delta in CR)"], index=0, horizontal=True)
    with col5:
        allocation = st.slider("Traffic Allocation to Experiment", 0.1, 1.0, 1.0, step=0.05)

    baseline_cr = form_data.get("baseline_cr", 0.0)
    weekly_sessions = form_data.get("weekly_sessions", 0)
...
def create_test_page():
    ...
    with colA:
        if st.button("Add to Catalog", use_container_width=True):
            add_to_catalog(form_data)
            st.success("Added to catalog below.")
    with colB:
        if st.button("Download as Word (.docx)", use_container_width=True):
            content = export_word_doc(form_data)
            if content:
                st.download_button("Save .docx", data=content, file_name=f"{form_data.get('test_id','test')}.docx", mime="application/vnd.openxmlformats-officedocument.wordprocessingml.document")
            else:
                st.error("python-docx not available in this environment.")
    with colC:
        st.write("")
...
def validate_page():
    ...
    if "last_form_data" not in st.session_state:
        st.info("No form data found. Fill the form in 'Create Test' first, then click 'Validate & Estimate Power/Duration'.")
        return
    form_data = st.session_state.last_form_data
    st.markdown("Using last form submission:")
    st.json({k: v for k, v in form_data.items() if k not in ["ready_checks"]})

    st.divider()
    st.markdown("### Re-run Power & Duration (optional)")
    power_results = validate_block(form_data)
    form_data["power"] = power_results
...
def catalog_page():
    ...
    if len(df) and st.button("Export Selected Row", use_container_width=True):
        ...
        # Map df row back to form-like structure for export
        form_like = {
...
def admin_page():
    st.header("Admin")
    st.caption("Configure dropdowns and defaults (kept local to your session). Future enhancement: persist to a small config file.")
    st.write("Current Primary KPI options:", PRIMARY_KPIS)
    st.write("Current Secondary KPI options:", SECONDARY_KPIS)
    st.write("Current Test Types:", TEST_TYPES)
    st.write("Current Funnel Stages:", FUNNEL_STAGES)
    st.write("Status Options:", STATUS_OPTIONS)
    st.info("Weights for EASIER are equal in this version.")

    st.markdown("---")
    st.caption("OpenAI setup (optional): add OPENAI_API_KEY to your Streamlit Cloud secrets or local env var.")
...
path = Path('/mnt/data/cro_self_service_tool.py')
